...
uint16_t bcolor[8]={
  wbBLACK, 0x18C3, 0x18C0, 0x00C3, 0x1803, 0x0003, 0x00C0, 0x1800//wbDARKGREEN
};
...
void init_keys()
{
  pinMode(36,INPUT); pinMode(32,INPUT); pinMode(33,INPUT); pinMode(0,INPUT_PULLUP);
...
{
  return(key = digitalRead(33)*128+digitalRead(32)*64+digitalRead(36)*32+digitalRead(39)*16
        +digitalRead(35)*8+digitalRead(34)*4+digitalRead(0)*2+digitalRead(0)*1);
}
  ...
  for(i=0; i<20; i++)
    for(j=0; j<10; j++)
      if (offboard[i][j]!=board[i][j]) // dirty rectangle check
        display.fillRect(i*3+3, 31-j*3-3, 3, 3, bcolor[offboard[i][j]=board[i][j]]);
      else display.fillRect(i*3+3, 31-j*3-3, 3, 3, 0);
  */
  for(i=0; i<19; i++)
      for(j=0; j<10; j++)
...
void clear_board()
{
  ...
  display.fillRect(0, 1, 63, 30, wbBLACK);
}
...
{
  display.fillRect(0, 0, 5, 5, wbBLACK);
  for(i=0; i<4; i++) {
...
/*
void wbpro_tickerAlarm(uint32_t us)
{
  timerAlarmWrite(wbpro_timer, us, true);
}

void wbpro_tickerEnable()
{
  timerAlarmEnable(wbpro_timer);
}

void wbpro_tickerDisable()
{
  timerAlarmEnable(wbpro_timer);
}
*/
