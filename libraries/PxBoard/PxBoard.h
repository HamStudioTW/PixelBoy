...
#ifndef PxBOARD_COLOR_DEPTH
#define PxBOARD_COLOR_DEPTH 4
#endif

#if PxBOARD_COLOR_DEPTH > 8 || PxBOARD_COLOR_DEPTH < 1
#error "PxBOARD_COLOR_DEPTH must be 1 to 8 bits maximum"
#endif

// Defines how long we display things by default
#ifndef PxBOARD_DEFAULT_SHOWTIME
#define PxBOARD_DEFAULT_SHOWTIME 30
#endif

// Defines the speed of the SPI bus (reducing this may help if you experience noisy images)
#ifndef PxBOARD_SPI_FREQUENCY
#define PxBOARD_SPI_FREQUENCY 20000000
#endif
...
#define color_step (256 / PxBOARD_COLOR_DEPTH)
...
class PxBOARD : public Adafruit_GFX {
 public:
  inline PxBOARD(uint16_t width, uint16_t height,uint8_t LATCH, uint8_t OE, uint8_t A,uint8_t B);
  inline PxBOARD(uint16_t width, uint16_t height,uint8_t LATCH, uint8_t OE, uint8_t A,uint8_t B,uint8_t C);
  inline PxBOARD(uint16_t width, uint16_t height,uint8_t LATCH, uint8_t OE, uint8_t A,uint8_t B,uint8_t C,uint8_t D);
  inline PxBOARD(uint16_t width, uint16_t height,uint8_t LATCH, uint8_t OE, uint8_t A,uint8_t B,uint8_t C,uint8_t D,uint8_t E);
 ...
 // the display buffer for the LED matrix
  uint8_t *PxBOARD_buffer;
#ifdef PxBOARD_DOUBLE_BUFFER
  uint8_t *PxBOARD_buffer2;
#endif
...
inline uint16_t PxBOARD::color565(uint8_t r, uint8_t g, uint8_t b) {
  return ((r & 0xF8) << 8) | ((g & 0xFC) << 3) | (b >> 3);
}

// Init code common to both constructors
inline void PxBOARD::init(uint16_t width, uint16_t height,uint8_t LATCH, uint8_t OE, uint8_t A, uint8_t B){
  _LATCH_PIN = LATCH;
  _OE_PIN = OE;
  _display_color=0;
  ...
  _mux_delay_E=0;
    ...
    PxBOARD_buffer= new uint8_t[PxBOARD_COLOR_DEPTH*_buffer_size];
  #ifdef PxBOARD_DOUBLE_BUFFER
    PxBOARD_buffer2=new uint8_t[PxBOARD_COLOR_DEPTH*_buffer_size];
  #endif

}

#ifdef ESP32
inline void PxBOARD::fm612xWriteRegister(uint16_t reg_value, uint8_t reg_position)
{
    spi_t * spi = SPI.bus();
...
#else
inline void PxBOARD::writeRegister(uint16_t reg_value, uint8_t reg_position)
{
...
inline void PxBOARD::setDriverChip(driver_chips driver_chip)
{
...
inline void PxBOARD::setMuxPattern(mux_patterns mux_pattern)
{
...
inline void PxBOARD::setColorOrder(color_orders color_order)
{
...
inline void PxBOARD::setMuxDelay(uint8_t mux_delay_A, uint8_t mux_delay_B, uint8_t mux_delay_C, uint8_t mux_delay_D, uint8_t mux_delay_E)
{
...
inline void PxBOARD::setScanPattern(scan_patterns scan_pattern)
{
  _scan_pattern=scan_pattern;
}

inline void PxBOARD::setPanelsWidth(uint8_t panels) {
  _panels_width=panels;
  _panel_width_bytes = (_width/_panels_width)/8;
}

inline void PxBOARD::setRotate(bool rotate) {
  _rotate=rotate;
}

inline void PxBOARD::setFlip(bool flip) {
  _flip=flip;
}

inline void PxBOARD::setFastUpdate(bool fast_update) {
  _fast_update=fast_update;
}

inline void PxBOARD::setBrightness(uint8_t brightness) {
  _brightness=brightness;
}


inline PxBOARD::PxBOARD(uint16_t width, uint16_t height,uint8_t LATCH, uint8_t OE, uint8_t A,uint8_t B) : Adafruit_GFX(width+ADAFRUIT_GFX_EXTRA, height)
{
  init(width, height, LATCH, OE, A, B);
}

inline PxBOARD::PxBOARD(uint16_t width, uint16_t height,uint8_t LATCH, uint8_t OE, uint8_t A,uint8_t B,uint8_t C) : Adafruit_GFX(width+ADAFRUIT_GFX_EXTRA, height)
{
  _C_PIN = C;
  init(width, height, LATCH, OE, A, B);
}

inline PxBOARD::PxBOARD(uint16_t width, uint16_t height,uint8_t LATCH, uint8_t OE, uint8_t A,uint8_t B,uint8_t C,uint8_t D) : Adafruit_GFX(width+ADAFRUIT_GFX_EXTRA, height)
{
  _C_PIN = C;
  _D_PIN = D;
  init(width, height, LATCH, OE, A, B);
...
inline PxBOARD::PxBOARD(uint16_t width, uint16_t height,uint8_t LATCH, uint8_t OE, uint8_t A,uint8_t B,uint8_t C,uint8_t D, uint8_t E) : Adafruit_GFX(width+ADAFRUIT_GFX_EXTRA, height)
{
  _C_PIN = C;
  _D_PIN = D;
  _E_PIN = E;
...
inline void PxBOARD::drawPixel(int16_t x, int16_t y, uint16_t color) {
  drawPixelRGB565(x, y, color);
}

inline void PxBOARD::showBuffer() {
  _active_buffer=!_active_buffer;
}

#ifdef PxBOARD_DOUBLE_BUFFER
inline void PxBOARD::copyBuffer(bool reverse = false) {
  // This copies the display buffer to the drawing buffer (or reverse)
  // You may need this in case you rely on the framebuffer to always contain the last frame
  // _active_buffer = true means that PxBOARD_buffer2 is displayed
  if(_active_buffer ^ reverse) {
    memcpy(PxBOARD_buffer, PxBOARD_buffer2, PxBOARD_COLOR_DEPTH*_buffer_size);
  } else {
    memcpy(PxBOARD_buffer2, PxBOARD_buffer, PxBOARD_COLOR_DEPTH*_buffer_size);
  }
}
#endif

inline void PxBOARD::setColorOffset(uint8_t r, uint8_t g,uint8_t b)
{
...
inline void PxBOARD::fillMatrixBuffer(int16_t x, int16_t y, uint8_t r, uint8_t g, uint8_t b,bool selected_buffer)
{
...
uint8_t *PxBOARD_bufferp = PxBOARD_buffer;

#ifdef PxBOARD_DOUBLE_BUFFER
  PxBOARD_bufferp = selected_buffer ? PxBOARD_buffer2 : PxBOARD_buffer;
#endif

  r = r >> (8-PxBOARD_COLOR_DEPTH);
  g = g >> (8-PxBOARD_COLOR_DEPTH);
  b = b >> (8-PxBOARD_COLOR_DEPTH);
  
  //Color interlacing
  for (int this_color_bit=0; this_color_bit<PxBOARD_COLOR_DEPTH; this_color_bit++)
  {
    if ((r >> this_color_bit) & 0x01)
      PxBOARD_bufferp[this_color_bit*_buffer_size+total_offset_r] |=_BV(bit_select);
    else
      PxBOARD_bufferp[this_color_bit*_buffer_size+total_offset_r] &= ~_BV(bit_select);

    if ((g >> this_color_bit) & 0x01)
      PxBOARD_bufferp[this_color_bit*_buffer_size+total_offset_g] |=_BV(bit_select);
    else
      PxBOARD_bufferp[this_color_bit*_buffer_size+total_offset_g] &= ~_BV(bit_select);

    if ((b >> this_color_bit) & 0x01)
      PxBOARD_bufferp[this_color_bit*_buffer_size+total_offset_b] |=_BV(bit_select);
    else
      PxBOARD_bufferp[this_color_bit*_buffer_size+total_offset_b] &= ~_BV(bit_select);
  }
}

inline void PxBOARD::drawPixelRGB565(int16_t x, int16_t y, uint16_t color) {
...
inline void PxBOARD::drawPixelRGB888(int16_t x, int16_t y, uint8_t r, uint8_t g,uint8_t b) {
#ifdef PxBOARD_DOUBLE_BUFFER
  fillMatrixBuffer(x, y, r, g, b, !_active_buffer);
#else
  fillMatrixBuffer(x, y, r, g, b, false);
...
inline uint8_t PxBOARD::getPixel(int8_t x, int8_t y) {
  return (0);//PxBOARD_buffer[x+ (y/8)*LCDWIDTH] >> (y%8)) & 0x1;
}

inline void PxBOARD::begin()
{
  begin(8);

}

void PxBOARD::begin(uint8_t row_pattern, uint8_t CLK, uint8_t MOSI, uint8_t MISO, uint8_t SS)
{
...
void PxBOARD::spi_init(){
    ...
    SPI.setFrequency(PxBOARD_SPI_FREQUENCY);
  #endif
...
void PxBOARD::begin(uint8_t row_pattern) {
...
void PxBOARD::set_mux(uint8_t value, boolean random_access = false)
{
...
void PxBOARD::latch(uint16_t show_time )
{
 ...
 void PxBOARD::display()
{
  display(PxBOARD_DEFAULT_SHOWTIME);
}

void PxBOARD::display(uint16_t show_time) {
  if (show_time == 0)
    show_time =1;
...
uint8_t *PxBOARD_bufferp = PxBOARD_buffer;
...
#ifdef PxBOARD_DOUBLE_BUFFER
  PxBOARD_bufferp = _active_buffer ? PxBOARD_buffer2 : PxBOARD_buffer;
#endif
    ...
    if(_driver_chip == SHIFT) {
      if ((_fast_update)&&(_brightness==255)){
        ...
        if (i<_row_pattern-1)
        {
          ...
          SPI_TRANSFER(&PxBOARD_bufferp[_display_color*_buffer_size+(i+1)*_send_buffer_size],_send_buffer_size);
        }
        else 
        { 
          // This pre-buffers the data for the first row pattern of the next _display_color
          SPI_TRANSFER(&PxBOARD_bufferp[((_display_color+1)%PxBOARD_COLOR_DEPTH)*_buffer_size],_send_buffer_size); 
        }
  ...
  {
    this_byte=PxBOARD_bufferp[_display_color*_buffer_size+i*_send_buffer_size+byte_cnt];
    SPI_BYTE(this_byte);
  }
#else
  SPI_TRANSFER(&PxBOARD_bufferp[_display_color*_buffer_size+i*_send_buffer_size],_send_buffer_size);
#endif
        latch(latch_time); 
      }
    }
    ...
    if (_driver_chip == FM6124 || _driver_chip == FM6126A) // _driver_chip == FM6124
    {
      ...
      uint8_t* bf = &PxBOARD_bufferp[_display_color*_buffer_size+i*_send_buffer_size];
      ...
      #endif
      SPI_TRANSFER(&PxBOARD_bufferp[_display_color*_buffer_size+i*_send_buffer_size],_send_buffer_size-1);
      pinMode(_SPI_CLK, OUTPUT);
      pinMode(_SPI_MOSI, OUTPUT);
      ...
      uint8_t v = PxBOARD_bufferp[_display_color*_buffer_size+i*_send_buffer_size + _send_buffer_size - 1];
  ...
  _display_color++;
  if (_display_color>=PxBOARD_COLOR_DEPTH)
  {
    _display_color=0;
  }
}

void PxBOARD::flushDisplay(void) {
  for (int ii=0;ii<_send_buffer_size;ii++)
    SPI_BYTE(0x00);
}

void PxBOARD::displayTestPattern(uint16_t show_time) {
...
void PxBOARD::displayTestPixel(uint16_t show_time) {
  if ((millis()-_test_last_call)>500)
  {
    flushDisplay();
...
void PxBOARD::clearDisplay(void) {
#ifdef PxBOARD_DOUBLE_BUFFER
...
// clear everything
void PxBOARD::clearDisplay(bool selected_buffer) {
#ifdef PxBOARD_DOUBLE_BUFFER
  if(selected_buffer)
    memset(PxBOARD_buffer2, 0, PxBOARD_COLOR_DEPTH*_buffer_size);
  else
    memset(PxBOARD_buffer, 0, PxBOARD_COLOR_DEPTH*_buffer_size);
#else
    memset(PxBOARD_buffer, 0, PxBOARD_COLOR_DEPTH*_buffer_size);
#endif
}
